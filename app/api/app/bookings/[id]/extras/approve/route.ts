...
export async function POST(req: Request, context: { params: { id: string } }) {
  ...
  if (!parsed.success) {
    return badRequest("INVALID_INPUT", "Invalid extras approval payload.");
  }
  ...
  let updatedCard = await JobCardsRepo.decideAdditionalCharge(card.id, charge.id, "APPROVED", auth.profile.id);
  if (!updatedCard) {
    return notFound("NOT_FOUND", "Job card not found.");
  }
  ...
  await logAudit({
    ...
    action: "job_card.extra.approved",
    entity: "job_card",
  ...
  await logNotificationEvent({
    eventType: "job_card.extra.approved",
    channel: "SYSTEM_STUB",
    message: `Client approved extra charge on ${updatedCard.ref}.`,
