...
import { badRequest, badRequestFromZod, created as createdResponse, notFound, ok, serviceUnavailable, tooManyRequests } from "@/lib/api/responses";
...
export async function POST(request: Request) {
  ...
  if (await isKillSwitchActive(KILL_SWITCHES.PUBLIC_BOOKING)) {
    return serviceUnavailable('Booking is temporarily unavailable')
  }
  ...
  if (!bookingRateCheck.allowed) {
    return tooManyRequests('RATE_LIMITED', 'Too many booking attempts. Please try again later.')
  }
